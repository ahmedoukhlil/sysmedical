...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Abonnement — {{ $cabinet->NomCabFr }}</h1>
        <a href="{{ route('admin.cabinets.export', $cabinet->idEntete) }}" class="bg-green-700 text-white px-4 py-2 rounded hover:bg-green-800">
            Exporter les données
        </a>
    </div>

    <div class="bg-white rounded-lg shadow p-6 mb-6 text-sm">
        <div class="text-gray-500">Stockage utilisé</div>
        <div class="text-lg font-semibold">
            {{ round($usedBytes / 1024 / 1024, 1) }} Mo
            @if ($quotaBytes !== null)
                / {{ round($quotaBytes / 1024 / 1024, 1) }} Mo
            @else
                / illimité
            @endif
        </div>
